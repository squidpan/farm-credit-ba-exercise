---
id: US-E6-203
type: user-story
status: draft

feature: FEATURE-E6-02

categories:
  - "[[Stories]]"
  - "[[Requirements]]"

tags:

description: Reserve sufficient funding capacity for a validated planned issuance.

created:
updated:
---

# US-E6-203 Reserve Funding Capacity

## TL;DR

Reserve sufficient funding capacity before the planned issuance can be booked.

---

# User Story

As a Funding Operations Analyst,

I want to reserve funding capacity for a validated issuance,

so that the planned issuance aligns with funding strategy and available liquidity.

---

# Business Value

Funding capacity reservation ensures that planned debt issuances remain within approved funding objectives before market execution.

---

# Scope

## Included

- Verify funding capacity
- Reserve available funding capacity
- Record reservation

## Excluded

- Security identifier assignment
- Booking the planned issuance
- Market execution

---

# Related Artifacts

## Parent Feature

- [[FEATURE-E6-02-schedule-and-book-issuance]]

## Acceptance Criteria

- [[AC-E6-203]]

## Business Rules

- [[BR-FEATURE-E6-02]]

---

# Notes
